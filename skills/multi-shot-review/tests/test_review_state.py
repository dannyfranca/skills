...
class CliTests(unittest.TestCase):
    ...
    def test_skill_documents_report_and_barrier_protocols(self) -> None:
        ...
        self.assertIn("## Joining long runs", text)
        self.assertIn("Run one review wave, then join it", text)
        self.assertIn("Complete when you hold the wave's final JSON", text)
        self.assertIn("keep the review target _frozen_", normalized)
        self.assertNotIn("exclusively in the foreground", text)
